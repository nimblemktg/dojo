...
vc_map(array(
        "name"                    => esc_html__("Red Carousel", 'givingwalk'),
        "base"                    => "red_carousel",
        "class"                   => "red-carousel",
        ...
        "description"             => esc_html__("Add Carousel", 'givingwalk'),
        "as_parent"               => array(
            'except' => 'red_carousel, red_blog, red_team, red_testimonial,red_clients, red_posts, vc_section, vc_row, vc_row_inner, vc_tta_accordion, vc_tta_tabs, vc_tta_tour, vc_tta_pageable , vc_tta_section, vc_basic_grid, vc_media_grid, vc_masonry_grid, vc_masonry_media_grid, vc_posts_slider, vc_images_carousel, vc_gallery, vc_widget_sidebar, vc_raw_js, vc_raw_html, vc_accordion, vc_tour, vc_tabs'
        ),
        ...
        "params"                  => array_merge(
            ...
            givingwalk_owl_settings()
        )
    )
);


class WPBakeryShortCode_red_carousel extends WPBakeryShortCodesContainer
{
    protected function content($atts, $content = null)
    {
        ...
        wp_enqueue_style('animate-css');
        givingwalk_owl_call_settings($atts);
        $content = $this->givingwalk_columnize_content($content);
        return parent::content($atts, $content);
    }

    protected function givingwalk_columnize_content($content)
    {
        $tag_pattern = '@\[([^<>&/\[\]\x00-\x20=]++)@';
    ...
    {
        $sortable = (vc_user_access_check_shortcode_all('red_carousel_group') ? 'wpb_sortable' : 'vc-non-draggable');

        return 'data-element_type="red_carousel" class="wpb_vc_tta_accordion wpb_sortable wpb_content_holder vc_shortcodes_container vc_tta-container vc_tta-o-non-responsive"' . $this->customAdminBlockParams();
    }
...
//                $column_controls = str_replace( 'vc_control-btn-clone"', 'vc_control-btn-clone" data-vc-control="clone"', $column_controls );
    ...
    public function contentAdmin($atts, $content = null)
    {
        ...
        for ($i = 0; $i < count($width); $i++) {
            ...
            $output .= '<div class="wpb_element_wrapper red-carousel">';
