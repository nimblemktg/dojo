<?php 
    ...
    $html_id = 'red-empty-space-'.uniqid(true);
    $values = (array) vc_param_group_parse_atts( $atts['values'] );
    $pattern = '/^(\d*(?:\.\d+)?)\s*(px|\%|in|cm|mm|em|rem|ex|pt|pc|vw|vh|vmin|vmax)?$/';
    $class = $css_class = $red_empty_space_css = '';
    foreach($values as $value){
        ...
        if(isset($screen_size) && !empty($screen_size)){
            ...
            $value_screen_size_width  = isset( $matches[1] ) ? (float) $matches[1] : (float) WPBMap::getParam( 'red_empty_space', $screen_size );
            ...
            $value_screen_size_height  = isset( $matches[1] ) ? (float) $matches[1] : (float) WPBMap::getParam( 'red_empty_space', $screen_size_height );
            ...
            $red_empty_space_css .= ' @media (max-width: '.$screen_size_width.'){'.$height_screen_size_css.' {height:'.$height_screen_size.';}}';
        }
    }
    ?>
<div id="<?php echo esc_attr($html_id);?>"  class="red-custom-css <?php echo esc_attr($css_class);?> mycolor">
    <style><?php echo wp_kses_post($red_empty_space_css);?></style>
</div>